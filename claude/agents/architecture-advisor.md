...
#### Layer-first推奨時の補足ガイダンス

feature横断の共有度が高い場合（シナリオ4: feature横断ビジネスロジック のスコアが高い）、
Cross-feature Serviceパターンの適用を推奨として提示する:
- 主featureの domain/ に Service interface を定義
- use_case/ で他featureの Repository interface を注入してオーケストレーション
- 詳細は `flutter-layer-first-architect.md` の「Cross-feature Service」セクションを参照
