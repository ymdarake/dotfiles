...
5. **型チェック**: `getDiagnostics()` を URI 指定なしで実行し、全ファイルの型エラーが 0 件であることを確認
   - エラーがある場合はコミットせず、エラー内容をユーザーに報告して修正を提案
6. gitmoji付きのコミットメッセージを作成して**ユーザーに提示**
7. コミット実行
8. コミット結果を見やすく表示
9. リモートリポジトリへプッシュ
...
- **テスト全パスでも型エラーが残るケースがある**（モックが独立しているため）。`getDiagnostics()` の全ファイルチェックで補完する
